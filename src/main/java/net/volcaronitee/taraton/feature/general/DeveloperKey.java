...
public class DeveloperKey {
    ...
    public static void register() {
        devKeybind = KeyBindingHelper.registerKeyBinding(new KeyBinding("Dev Key",
                InputUtil.Type.KEYSYM, InputUtil.UNKNOWN_KEY.getCode(), "Taraton"));
    ...
    private void copyNbtToClipboard(MinecraftClient client, NbtElement nbt, String dataType) {
        ...
        try {
            ...
            String formattedJson = TaratonJson.GSON.toJson(jsonElement);
            client.keyboard.setClipboard(formattedJson);

            Taraton.sendMessage(Text.literal("Copied " + dataType + " data to clipboard.")
                    .formatted(Formatting.GREEN));
        } catch (Exception e) {
            Taraton.sendMessage(Text.literal("Failed to copy " + dataType + " data to clipboard.")
        ...
        while (devKeybind.wasPressed()) {
            ...
            if (crosshairTarget.getType() == HitResult.Type.ENTITY) {
                ...
                if (blockEntity != null) {
                    ...
                    if (result.isPresent()) {
                        String formattedJson = TaratonJson.GSON.toJson(result.get());
                        client.keyboard.setClipboard(formattedJson);
                        Taraton.sendMessage(Text.literal("Copied Block State data to clipboard.")
                                .formatted(Formatting.GREEN));
                    } else {
                        Taraton.sendMessage(
